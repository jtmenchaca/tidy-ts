...
import type { PivotLongerMethod, PivotWiderMethod } from "../../verbs/reshape/pivot-types.ts";
import type { TransposeMethod } from "../../verbs/reshape/transpose.types.ts";
import type { ForEachColAsyncMethod, ForEachColMethod, ForEachRowAsyncMethod, ForEachRowMethod } from "../../verbs/utility/for-each.types.ts";
...
import type { SliceHeadMethod, SliceMaxMethod, SliceMinMethod, SliceRowsMethod, SliceSampleMethod, SliceTailMethod } from "../../verbs/filtering/slice.types.ts";
import type { UngroupMethod } from "../../verbs/grouping/ungroup.types.ts";
import type { AsofJoinMethod, CrossJoinMethod, InnerJoinMethod, LeftJoinMethod, OuterJoinMethod, RightJoinMethod } from "../../verbs/join/types/index.ts";
import type { GroupByMethod } from "../../verbs/grouping/group-by.types.ts";
import type { ExtractHeadMethod, ExtractMethod, ExtractNthMethod, ExtractSampleMethod, ExtractTailMethod, ExtractUniqueMethod } from "../../verbs/selection/extract.types.ts";
...
import type { ReplaceNaMethod, ReplaceNullMethod, ReplaceUndefinedMethod } from "../../verbs/missing-data/replace-na.types.ts";
...
export type DataFrameColumns<Row extends object> = {
    [ColName in Extract<keyof Row, string> as ColName extends "nrows" | "columns" | "groupKeys" | "isGrouped" | "get" | "has" ? never : ColName]-?: readonly Row[ColName][];
};
/**
 ...
 */
export interface DataFrameBase<Row extends object = object> extends Forbid<ForbiddenArrayMethods> {
 ...
 */
export type DataFrame<Row extends object = object> = DataFrameBase<Row> & DataFrameColumns<Row>;
/** DataFrameWithRowLabels adds row label metadata while keeping the DataFrame surface. */
export type DataFrameWithRowLabels<Row extends object, Labels extends readonly RowLabel[] = readonly RowLabel[]> = DataFrame<Row> & {
    __rowLabels: Labels;
};
/**
 ...
 * ```
 */
export type GroupedDataFrame<Row extends object, GroupKeys extends keyof Row = keyof Row> = DataFrame<Row> & {
    __groups: {
