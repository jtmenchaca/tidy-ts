...
import type { RestrictEmptyDataFrame } from "../../dataframe/types/error-types.ts";
/**
 * Helper type to extract the element type from an array type
 * Handles both T[] and T[] | null
 */
type ArrayElement<T> = T extends (infer U)[] ? U : T extends (infer U)[] | null ? U : never;
/**
...
type ArrayColumns<Row extends object> = {
    [K in keyof Row]: Row[K] extends unknown[] ? K : Row[K] extends (infer U)[] | null ? K : never;
}[keyof Row];
/**
 ...
 */
type EmptyDataFrameUnnest = "🚨 Cannot unnest empty DataFrame - no columns available! Try adding data first with createDataFrame([...rows])";
/**
...
export type UnnestMethod<Row extends object> = {
     ...
     * ```
     */
    <R extends object, Col extends ArrayColumns<R>>(this: DataFrame<R>, column: RestrictEmptyDataFrame<R, Col, EmptyDataFrameUnnest>): DataFrame<UnnestColumn<R, Col>>;
    /**
     ...
     * ```
     */
    <R extends object, Cols extends readonly ArrayColumns<R>[]>(this: DataFrame<R>, columns: RestrictEmptyDataFrame<R, Cols, EmptyDataFrameUnnest>): DataFrame<UnnestMultipleColumns<R, Cols>>;
};
/**
 * Helper type to unnest multiple columns
 */
type UnnestMultipleColumns<Row, Cols extends readonly (keyof Row)[]> = Cols extends readonly [infer First, ...infer Rest] ? First extends keyof Row ? Rest extends readonly (keyof Row)[] ? UnnestColumn<UnnestMultipleColumns<Row, Rest>, First> : UnnestColumn<Row, First> : Row : Row;
export {};
