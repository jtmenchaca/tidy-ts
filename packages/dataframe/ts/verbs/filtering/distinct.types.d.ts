...
export type DistinctMethod<Row extends object> = {
     ...
     * df.groupBy("year").distinct("product")
     */
    <R extends object, GroupName extends keyof R, Cols extends keyof R>(this: GroupedDataFrame<R, GroupName>, column1: Cols, ...moreColumns: Cols[]): PreserveGrouping<R, GroupName, {
        [K in Cols]: R[K];
    }>;
    /**
     * Get unique combinations of specified columns (SQL DISTINCT).
     ...
     * df.groupBy("year").distinct("product")
     */
    <R extends object, Cols extends keyof R>(this: DataFrame<R>, column1: Cols, ...moreColumns: Cols[]): DataFrame<{
        [K in Cols]: R[K];
    }>;
};
