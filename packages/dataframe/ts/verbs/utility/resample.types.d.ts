...
import type { ColumnTypeMismatchDate, EmptyDataFrameResample, RestrictEmptyDataFrame, ValidateDateColumn } from "../../dataframe/types/error-types.ts";
/**
 ...
 */
export type FillMethod = (values: unknown[]) => unknown[];
/**
...
export type ResampleOptions<T extends object> = {
    [K: string]: AggregationFunction<T> | FillMethod | number | null | undefined;
    method?: FillMethod;
} | {
    method?: FillMethod;
};
 ...
 */
type AggregationReturnType<Fn> = Fn extends (...args: any[]) => infer R ? R : unknown;
/**
 * Infer the return type for a fill method.
 * Fill methods return arrays, but we extract a single value, so we use unknown.
 */
type FillReturnType<Fn> = Fn extends (values: unknown[]) => infer R ? R extends unknown[] ? R[number] : unknown : unknown;
/**
 * Named arguments for resample method.
 */
export type ResampleArgs<Row extends object, TimeCol extends keyof Row, Options extends ResampleOptions<Row>> = {
    timeColumn: RestrictEmptyDataFrame<Row, ValidateDateColumn<Row, TimeCol, ColumnTypeMismatchDate>, EmptyDataFrameResample>;
    frequency: Frequency;
    metrics: Options;
    startDate?: Date;
    endDate?: Date;
...
export type ResampleMethod<Row extends object> = {
     ...
     * });
     */
    <R extends object, GroupName extends keyof R, TimeCol extends keyof R, Options extends ResampleOptions<R>>(this: GroupedDataFrame<R, GroupName>, args: ResampleArgs<R, TimeCol, Options>): DataFrame<{
        [K in GroupName | TimeCol | Exclude<keyof Options, "method" | TimeCol> | (Options extends {
            method?: FillMethod;
        } ? Exclude<keyof R, TimeCol> : never)]: K extends TimeCol ? R[TimeCol] : K extends GroupName ? R[K] : K extends Exclude<keyof Options, "method" | TimeCol> ? Options[K] extends AggregationFunction<R> ? AggregationReturnType<Options[K]> : Options[K] extends FillMethod ? FillReturnType<Options[K]> : Options[K] extends number | null ? Options[K] : unknown : Options extends {
            method?: FillMethod;
        } ? K extends keyof R ? FillReturnType<Options["method"]> | undefined : never : never;
    }>;
    /**
     ...
     * });
     */
    <R extends object, TimeCol extends keyof R, Options extends ResampleOptions<R>>(this: DataFrame<R>, args: ResampleArgs<R, TimeCol, Options>): DataFrame<{
        [K in TimeCol | Exclude<keyof Options, "method" | TimeCol> | (Options extends {
            method?: FillMethod;
        } ? Exclude<keyof R, TimeCol> : never)]: K extends TimeCol ? R[TimeCol] : K extends Exclude<keyof Options, "method" | TimeCol> ? Options[K] extends AggregationFunction<R> ? AggregationReturnType<Options[K]> : Options[K] extends FillMethod ? FillReturnType<Options[K]> : Options[K] extends number | null ? Options[K] : unknown : Options extends {
            method?: FillMethod;
        } ? K extends keyof R ? FillReturnType<Options["method"]> | undefined : never : never;
    }>;
};
