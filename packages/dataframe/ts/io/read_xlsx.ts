...
export async function parseXLSXRaw(
  ...
  // Get relationship mappings to correctly map rId to sheet paths
...
function parseWorkbookSheets(
  ...
  // Parse relationship mappings from workbook.xml.rels if available
  ...
  if (relsXml) {
    ...
    let relsMatch;
    while ((relsMatch = relsRegex.exec(relsXml)) !== null) {
      const id = relsMatch[1];
      let target = relsMatch[2];
      // Normalize target path (remove leading ../ or /)
      if (target.startsWith("/")) {
        target = target.slice(1);
      }
      // Prepend xl/ if not already present and it's a worksheet
  ...
  // Match sheet elements - handle both attribute orders
  // <sheet name="..." r:id="..."/> or <sheet r:id="..." name="..."/>
  ...
  while ((match = sheetRegex.exec(workbookXml)) !== null) {
    ...
    // Extract name and r:id from attributes
    ...
    // Look up the actual path from relationships
    let path: string;
    if (relsMap.has(rId)) {
      path = relsMap.get(rId)!;
    } else {
      // Fallback: assume sequential sheet numbering based on order in workbook
      // This is a last resort when rels file is not available
