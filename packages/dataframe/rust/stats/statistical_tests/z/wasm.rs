//! WASM bindings for z-tests

#![cfg(any(feature = "wasm", feature = "napi-rs"))]

use super::{one_sample::z_test, sample_size::z_sample_size, two_sample::z_test_ind};
use crate::stats::helpers::parse_alternative;
#[cfg(feature = "wasm")]
use wasm_bindgen::prelude::*;

/// WASM export for one-sample z-test
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn z_test_one_sample(
    x: &[f64],
    mu: f64,
    sigma: f64,
    alpha: f64,
    alternative: &str,
) -> Result<JsValue, JsValue> {
    let alternative_type = parse_alternative(alternative);
    let result = z_test(x.iter().copied(), mu, sigma, alternative_type, alpha)
        .map_err(|e| JsValue::from_str(&e))?;
    serde_wasm_bindgen::to_value(&result)
        .map_err(|e| JsValue::from_str(&e.to_string()))
}

/// WASM export for two-sample z-test
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn z_test_two_sample(
    x: &[f64],
    y: &[f64],
    sigma_x: f64,
    sigma_y: f64,
    alpha: f64,
    alternative: &str,
) -> Result<JsValue, JsValue> {
    let alternative_type = parse_alternative(alternative);
    let result = z_test_ind(
        x.iter().copied(),
        y.iter().copied(),
        sigma_x,
        sigma_y,
        alternative_type,
        alpha,
    )
    .map_err(|e| JsValue::from_str(&e))?;
    serde_wasm_bindgen::to_value(&result)
        .map_err(|e| JsValue::from_str(&e.to_string()))
}

/// WASM export for z-test sample size calculation
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn z_sample_size_wasm(effect_size: f64, alpha: f64, power: f64, test_type: &str) -> f64 {
    let alternative_type = parse_alternative(test_type);
    z_sample_size(effect_size, alpha, power, 1.0, alternative_type) // Using std_dev = 1.0 as default
}

// --- NAPI-RS exports ---

#[cfg(feature = "napi-rs")]
use napi_derive::napi;

#[cfg(feature = "napi-rs")]
#[napi]
pub fn z_test_one_sample_napi(
    x: &[f64],
    mu: f64,
    sigma: f64,
    alpha: f64,
    alternative: String,
) -> Result<String, napi::Error> {
    let alternative_type = parse_alternative(&alternative);
    let result = z_test(x.iter().copied(), mu, sigma, alternative_type, alpha)
        .map_err(|e| napi::Error::from_reason(e))?;
    serde_json::to_string(&result)
        .map_err(|e| napi::Error::from_reason(e.to_string()))
}

#[cfg(feature = "napi-rs")]
#[napi]
pub fn z_test_two_sample_napi(
    x: &[f64],
    y: &[f64],
    sigma_x: f64,
    sigma_y: f64,
    alpha: f64,
    alternative: String,
) -> Result<String, napi::Error> {
    let alternative_type = parse_alternative(&alternative);
    let result = z_test_ind(
        x.iter().copied(),
        y.iter().copied(),
        sigma_x,
        sigma_y,
        alternative_type,
        alpha,
    )
    .map_err(|e| napi::Error::from_reason(e))?;
    serde_json::to_string(&result)
        .map_err(|e| napi::Error::from_reason(e.to_string()))
}

#[cfg(feature = "napi-rs")]
#[napi]
pub fn z_sample_size_napi(effect_size: f64, alpha: f64, power: f64, test_type: String) -> f64 {
    let alternative_type = parse_alternative(&test_type);
    z_sample_size(effect_size, alpha, power, 1.0, alternative_type)
}
