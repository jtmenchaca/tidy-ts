...
/// NAPI export for sum aggregation
#[cfg(feature = "napi-rs")]
#[napi]
pub fn reduce_sum_f64_napi(gid_per_row: &[u32], vals: &[f64], n_groups: u32) -> Vec<f64> {
    let g = n_groups as usize;
    let mut out = vec![0.0f64; g];
...
/// NAPI export for count aggregation
#[cfg(feature = "napi-rs")]
#[napi]
pub fn reduce_count_u32_napi(gid_per_row: &[u32], valid: &[u8], n_groups: u32) -> Vec<u32> {
    let mut out = vec![0u32; n_groups as usize];
    for i in 0..gid_per_row.len() {
        if valid[i] != 0 {
            out[gid_per_row[i] as usize] += 1;
...
/// NAPI export for mean aggregation
#[cfg(feature = "napi-rs")]
#[napi]
pub fn reduce_mean_f64_napi(gid_per_row: &[u32], vals: &[f64], valid: &[u8], n_groups: u32) -> Vec<f64> {
    let g = n_groups as usize;
    let mut sums = vec![0.0f64; g];
    let mut cnts = vec![0u32; g];
    for i in 0..vals.len() {
