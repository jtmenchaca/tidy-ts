//! Ultra-optimized outer join operation WASM/NAPI exports - using shared utilities
...
// 1 column (exact)
#[cfg(any(feature = "wasm", feature = "napi-rs"))]
fn outer_join_1col(left: &[u32], right: &[u32]) -> (Vec<u32>, Vec<u32>) {
    let (map, adj) = build_csr_from_keys_u32(right);

    let mut right_seen = vec![false; right.len()];
    ...
    for (i, &k) in left.iter().enumerate() {
        if let Some(off) = map.get(&k) {
            ...
            for pos in start..end {
                let right_idx = adj[pos] as usize;
                left_out[o] = i as u32;
                right_out[o] = right_idx as u32;
                right_seen[right_idx] = true;
                o += 1;
            }
        } else {
            left_out[o] = i as u32;
            right_out[o] = SENTINEL;
            o += 1;
        }
    }

    for (right_row, &seen) in right_seen.iter().enumerate() {
        if !seen {
            left_out.push(SENTINEL);
            right_out.push(right_row as u32);
        }
    }
...
#[cfg(any(feature = "wasm", feature = "napi-rs"))]
fn outer_join_2col(la: &[u32], lb: &[u32], ra: &[u32], rb: &[u32]) -> (Vec<u32>, Vec<u32>) {
    let n_left = la.len();
    let n_right = ra.len();
    ...
    let mut right_seen = vec![false; n_right];
...
#[cfg(any(feature = "wasm", feature = "napi-rs"))]
fn outer_join_multi(left_cols: &[&[u32]], right_cols: &[&[u32]]) -> (Vec<u32>, Vec<u32>) {
...
#[cfg(any(feature = "wasm", feature = "napi-rs"))]
fn outer_join_dispatch(left_cols: &[&[u32]], right_cols: &[&[u32]]) -> (Vec<u32>, Vec<u32>) {
    ...
    match num_cols {
        1 => outer_join_1col(&left_cols[0][..left_len], &right_cols[0][..right_len]),
        2 => {
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn outer_join_typed_multi_u32(
    ...
    let (left_idx, right_idx) = outer_join_dispatch(&lrefs, &rrefs);
    JoinIdxU32::new(left_idx, right_idx)
}

#[cfg(feature = "napi-rs")]
#[napi]
pub fn outer_join_typed_multi_u32_napi(
    ...
    let (left_idx, right_idx) = outer_join_dispatch(&left_columns, &right_columns);
