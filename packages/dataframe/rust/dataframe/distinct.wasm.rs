...
/// NAPI export for distinct rows — accepts array of Uint32Array columns (same API as WASM)
#[cfg(feature = "napi-rs")]
#[napi]
pub fn distinct_rows_generic_typed_napi(
    column_data: Vec<&[u32]>,
    view_index: &[u32],
) -> Vec<u32> {
    let n_cols = column_data.len();
    let mut hash_tbl = hashbrown::HashMap::with_capacity(view_index.len());
    let mut result_indices = Vec::with_capacity(view_index.len());

    for &physical_idx in view_index.iter() {
        let mut key_values = Vec::with_capacity(n_cols);
        for col in &column_data {
            key_values.push(col[physical_idx as usize]);
        }
