...
export const windowDocs: Record<string, DocEntry> = {
  ...
  lag: {
    ...
    examples: [
      ...
      "s.lag([1, 2, 3, 4, 5], { defaultValue: 0 })  // [0, 1, 2, 3, 4]",
      '// In mutate (ungrouped)',
      'df.mutate({ prev_sales: s.lag(df.extract("sales"), { defaultValue: 0 }) })',
      '// In mutate (grouped)',
      'df.groupBy("symbol").mutateOverGroup({ prev_price: (gdf) => s.lag(gdf.extract("price")) })',
    ],
    related: ["lead"],
  },
  ...
  lead: {
    ...
    examples: [
      ...
      "s.lead([1, 2, 3, 4, 5], { defaultValue: 0 })  // [2, 3, 4, 5, 0]",
      '// In mutate (ungrouped)',
      'df.mutate({ next_sales: s.lead(df.extract("sales"), { defaultValue: 0 }) })',
      '// In mutate (grouped)',
      'df.groupBy("symbol").mutateOverGroup({ next_price: (gdf) => s.lead(gdf.extract("price")) })',
    ],
