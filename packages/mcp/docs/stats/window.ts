...
export const windowDocs: Record<string, DocEntry> = {
  ...
  lag: {
    ...
    category: "stats",
    signature:
      "s.lag(values: T[], k?: number, defaultValue?: T): (T | undefined)[]",
    description:
    ...
    examples: [
      ...
      '// In mutate (ungrouped)',
      'df.mutate({ prev_sales: s.lag(df.extract("sales"), 1, 0) })',
      '// In mutate (grouped)',
      'df.groupBy("symbol").mutate({ prev_price: (_row, i, groupDf) => s.lag(groupDf.extract("price"), 1)[i] })',
    ],
    related: ["lead"],
  },
  ...
  lead: {
    ...
    category: "stats",
    signature:
      "s.lead(values: T[], k?: number, defaultValue?: T): (T | undefined)[]",
    description:
    ...
    examples: [
      ...
      '// In mutate (ungrouped)',
      'df.mutate({ next_sales: s.lead(df.extract("sales"), 1, 0) })',
      '// In mutate (grouped)',
      'df.groupBy("symbol").mutate({ next_price: (_row, i, groupDf) => s.lead(groupDf.extract("price"), 1)[i] })',
    ],
