...
function renderHeatmap(): string {
  ...
  for (let ci = 0; ci < DATA.length; ci++) {
    ...
    for (let ei = 0; ei < ECOS.length; ei++) {
      ...
      const pct = Math.round((caught / cat.n) * 100);
      svgBody += `<text x="${cx + cellW / 2}" y="${cy + cellH / 2 + 1}" text-anchor="middle" font-size="16" fill="${tc}" font-weight="700">${caught}</text>\n`;
      svgBody += `<text x="${cx + cellW / 2}" y="${cy + cellH / 2 + 15}" text-anchor="middle" font-size="9" fill="${tc}" opacity="0.8">(${pct}%)</text>\n`;
    }
  }
  // Totals
  ...
  for (let ei = 0; ei < ECOS.length; ei++) {
    ...
    const cx = ML + ei * cellW + cellW / 2;
    svgBody += `<text x="${cx}" y="${ty + 14}" text-anchor="middle" font-size="12" fill="#333" font-weight="700">${v} / 65 (${Math.round(v / 65 * 100)}%)</text>\n`;
  }
