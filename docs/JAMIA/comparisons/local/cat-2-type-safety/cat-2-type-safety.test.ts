...
const INTENTS = [
  "Multiply a lab test-name string column by 10.",
  "Compute the mean of a lab test-name string column.",
  "Filter labs where result value equals the string `\"high\"`.",
  "Convert a free-text result column to numeric, then aggregate.",
  "Multiply a converted-to-numeric column by 2 without narrowing nulls first.",
  "Compute the mean of a converted-to-numeric column.",
  "Multiply a column whose mutate returned `number | \"HIGH\"` by 2.",
  "Parse a date string column to a Date type.",
  "Filter encounters where admission date is greater than 100.",
  "Add 7 to a Date column to get a date one week later.",
  "Compute `Math.log(insurance_carrier)` on an insurance-type string column.",
  "Transpose a wide vitals table, then multiply a transposed row by 2.",
  "Transpose a wide patient table, then access a pre-transpose column name on the result.",
  "Filter encounters where status equals `\"unknown\"` when Status is a closed enum.",
];
