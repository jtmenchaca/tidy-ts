...
const INTENTS = [
  "Format a nullable lab reference range to two decimal places.",
  "Compute the difference between each lab result value and the upper reference range.",
  "Filter labs where the upper reference range is above 100.",
  "Compute the ratio of result value to upper reference range.",
  "Replace nulls in the reference range with a fallback, mutate to re-introduce nulls under a condition, then compute a ratio.",
  "Take the mean of a nullable lab column, then double the result.",
  "Sum a nullable lab column, then double the result.",
  "Take the min of a nullable lab column, then double the result.",
  "Group labs by patient, take the per-group mean of a nullable column, then double each.",
  "Sum a nullable column and use the total directly.",
  "Divide a sum of a nullable column by 2 to compute an average.",
  "Compute the prior visit's lab value for each row using `lag()`.",
  "Subtract each patient's prior visit lab value from the current visit.",
  "Sort labs by result value, take the top 5.",
  "Pivot vitals to one column per metric, then compute systolic minus diastolic.",
  "Distinguish 'lab result was null' from 'lab was not ordered' in downstream logic.",
  "Fill missing reference ranges with a default, distinguishing semantic null from absent field.",
];
...
Deno.test("Cat 3 — Null & Missing Data: Tidy-TS runtime", () => {
  ...
  // b: silent (JS coercion) — verify the corruption marker (null coerced or NaN propagated)
  expectSilentCorruption(tsResults[1], /null coerced to 0 silently|null propagated as NaN/, "3b runtime: arithmetic on nullable");
  // c: silent (null > 100 is false) — verify null rows were silently dropped
  expectSilentCorruption(tsResults[2], /null rows silently dropped/, "3c runtime: comparison on nullable");
  // d: silent (null coerces to 0, produces Infinity) — verify Infinity appeared
  expectSilentCorruption(tsResults[3], /Infinity from null div/, "3d runtime: arithmetic on nullable before narrowing");
