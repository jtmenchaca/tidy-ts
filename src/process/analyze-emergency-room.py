#!/usr/bin/env python3
"""
Emergency Room OCEL Analysis - Detailed Insights
Run with: cd src/process && poetry run python analyze-emergency-room.py
"""
...
print(f"Dataset Overview (Recoded - doctors/nurses as single objects):")
print(f"  - Total Events: {len(ocel.events)}")
print(f"  - Total Objects: {len(ocel.objects)}")
print(f"  - Total Relations: {len(ocel.relations)}")

# Create visualizations for recoded version
print("\n=== Creating Recoded Visualizations ===\n")
ocdfg = pm4py.discover_ocdfg(ocel)

try:
    freq_path = fixtures_dir / "er_recoded_ocdfg_frequency.png"
    pm4py.save_vis_ocdfg(ocdfg, str(freq_path), annotation="frequency")
    print(f"✓ Saved recoded frequency diagram: {freq_path}")
except Exception as e:
    print(f"✗ Frequency diagram failed: {e}")

try:
    perf_path = fixtures_dir / "er_recoded_ocdfg_performance.png"
    pm4py.save_vis_ocdfg(ocdfg, str(perf_path), annotation="performance")
    print(f"✓ Saved recoded performance diagram: {perf_path}")
except Exception as e:
    print(f"✗ Performance diagram failed: {e}")

try:
    ocpn = pm4py.discover_oc_petri_net(ocel)
    ocpn_path = fixtures_dir / "er_recoded_ocpn.png"
    pm4py.save_vis_ocpn(ocpn, str(ocpn_path))
    print(f"✓ Saved recoded Petri net: {ocpn_path}")
except Exception as e:
...
# Object lifecycles
print("\n" + "-"*70)
print("PATIENT LIFECYCLES (PROCESS FLOWS)")
print("-"*70)

patients = ocel.objects[ocel.objects['ocel:type'] == 'Patient']['ocel:oid'].tolist()

for patient in patients:
    print(f"\n{patient.upper()} Timeline:")

    # Get all events for this patient
    patient_relations = ocel.relations[ocel.relations['ocel:oid'] == patient]
    patient_event_ids = patient_relations['ocel:eid'].tolist()
    patient_events = ocel.events[ocel.events['ocel:eid'].isin(patient_event_ids)].sort_values('ocel:timestamp')

    for _, event in patient_events.iterrows():
        timestamp = event['ocel:timestamp']
        activity = event['ocel:activity']

        # Find other objects involved in this event
        event_relations = ocel.relations[ocel.relations['ocel:eid'] == event['ocel:eid']]
        other_objects = event_relations[event_relations['ocel:oid'] != patient]['ocel:oid'].tolist()

        if other_objects:
            print(f"  {timestamp} - {activity} (with {', '.join(other_objects)})")
        else:
            print(f"  {timestamp} - {activity}")

    # Calculate duration
    first_event = patient_events.iloc[0]['ocel:timestamp']
    last_event = patient_events.iloc[-1]['ocel:timestamp']
