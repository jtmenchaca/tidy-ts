...
impl WilcoxonWTest {
    ...
    pub fn paired(x: &[f64], y: &[f64], alpha: f64, alternative: &str) -> TestResult {
        ...
        let distribution = match SignedRank::new(d.len(), zeroes, tie_correction) {
            Ok(dist) => dist,
            Err(e) => {
                return create_error_result(
                    "Wilcoxon signed rank test",
                    &format!("Failed to create distribution: {}", e),
                );
            }
        };
        ...
        // Create TestResult
        let test_statistic = estimate_small; // W statistic
        let confidence_interval = (0.0, rank_sum); // Approximate CI
        let null_hypothesis = "H0: The distributions are identical".to_string();
        let alt_hypothesis = match alternative {
            "less" => "Ha: First distribution is stochastically less than second".to_string(),
            "greater" => "Ha: First distribution is stochastically greater than second".to_string(),
            _ => "Ha: The distributions are not identical".to_string(),
        };
        let reject_null = p_value < alpha;

        TestResult {
            test_type: TestType::WilcoxonSignedRank,
            ...
            w_statistic: Some(test_statistic),
            sample_size: Some(d.len()),
            tie_correction: Some(tie_correction as f64),
            ranks: Some(ranks),
            ..Default::default()
        }
    }
...
mod tests {
    ...
    fn paired() {
        ...
        let test = WilcoxonWTest::paired(&x, &y, 0.05, "two-sided");
