...
impl SignedRank {
    ...
    ///
    /// ```
    /// use tidy_ts_dataframe::stats::statistical_tests::wilcoxon::signed_rank::SignedRank;
    ///
    /// let result = SignedRank::new(25, 0, 0);
    /// assert!(result.is_ok());
    /// ```
...
impl rand::prelude::Distribution<f64> for SignedRank {
    fn sample<R: Rng + ?Sized>(&self, r: &mut R) -> f64 {
        match self.approximation {
            Approximation::Normal(normal) => {
                rand::prelude::Distribution::sample(&normal, r)
            }
            Approximation::Exact => {
                todo!();
            }
