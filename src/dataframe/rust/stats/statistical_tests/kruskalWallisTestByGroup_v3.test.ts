...
async function main() {
  console.log("Testing R vs Rust kruskalWallisTestByGroup function...\n");
  ...
  const testCases = [
    {
      testName: "kruskalWallisTestByGroup (α=0.05)",
      func: "kruskal.test.bygroup",
      distribution: "kruskal_wallis",
      args: [JSON.stringify(groups), "0.05"],
    },
    {
      testName: "kruskalWallisTestByGroup (α=0.01)",
      func: "kruskal.test.bygroup",
      distribution: "kruskal_wallis",
