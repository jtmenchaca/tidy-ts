use super::super::super::core::{
    AlternativeType,
    types::{OneSampleProportionTestResult, TwoSampleProportionTestResult, TestStatistic, TestStatisticName, ConfidenceInterval},
};
...
pub fn chi_square_test_one_sample(
    ...
    let chi_square_stat = 
        ((observed_success - expected_success).abs() - yates).powi(2) / expected_success +
    ...
    let p_upper = if p_c_upper >= 1.0 {
        ...
        (p_c_upper + z22n + z_crit * (p_c_upper * (1.0 - p_c_upper) / n + z22n / (2.0 * n)).sqrt()) / (1.0 + 2.0 * z22n)
    };
    ...
    let p_lower = if p_c_lower <= 0.0 {
        ...
        (p_c_lower + z22n - z_crit * (p_c_lower * (1.0 - p_c_lower) / n + z22n / (2.0 * n)).sqrt()) / (1.0 + 2.0 * z22n)
    };
...
pub fn chi_square_test_two_sample(
    ...
    if x1 < 0.0 || x1 > n1 || n1 <= 0.0 {
        return Err("Invalid counts for sample 1: x1 must be between 0 and n1, n1 must be positive".to_string());
    }
    
    if x2 < 0.0 || x2 > n2 || n2 <= 0.0 {
        return Err("Invalid counts for sample 2: x2 must be between 0 and n2, n2 must be positive".to_string());
    }
