//! GLM ANOVA format functions
//!
//! This file contains the format functions for GLM ANOVA.

use super::types::GlmAnova;

/// Format ANOVA result as string
///
/// This function formats an ANOVA result as a string without printing it.
///
/// # Arguments
///
/// * `anova` - ANOVA result to format
///
/// # Returns
///
/// A formatted string representation of the ANOVA result.
pub fn format_anova(anova: &GlmAnova) -> String {
    ...
    for (row_name, row) in anova.row_names.iter().zip(anova.table.iter()) {
        ...
        if let Some(df) = row.df {
            output.push_str(&format!("{:>12.1}", df));
        } else {
            output.push_str(&format!("{:>12}", ""));
        }
        ...
        if let Some(dev) = row.deviance {
            output.push_str(&format!("{:>12.3}", dev));
        } else {
            output.push_str(&format!("{:>12}", ""));
        }

        // Format residual df
        output.push_str(&format!("{:>12.0}", row.resid_df));

        // Format residual deviance
        output.push_str(&format!("{:>12.3}", row.resid_deviance));

        // Format additional columns if present
        if let Some(rao) = row.rao {
            output.push_str(&format!("{:>12.3}", rao));
        }
        if let Some(f_stat) = row.f_statistic {
            output.push_str(&format!("{:>12.3}", f_stat));
        }
        if let Some(p_val) = row.p_value {
            output.push_str(&format!("{:>12.3}", p_val));
        }
...
mod tests {
    ...
    #[test]
    fn test_format_anova() {
        let result = create_test_glm_result();
        let anova = anova_glm(&result, None, None).unwrap();
        let formatted = format_anova(&anova);

        assert!(formatted.contains("Analysis of Deviance Table"));
        assert!(formatted.contains("Df"));
        assert!(formatted.contains("Deviance"));
    }
}
