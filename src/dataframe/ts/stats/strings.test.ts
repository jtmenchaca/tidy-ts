...
Deno.test("String functions basic functionality", () => {
  ...
  // Test str_detect
  const hasHello = str.detect(strings, "hello") as boolean[];
...
Deno.test("String functions with regex patterns", () => {
  ...
  const hasNumbers = str.detect(strings, "\\d+") as boolean[];
  expect(hasNumbers).toEqual([true, false, true]);

  // Test regex extraction
  const firstNumber = str.extract(strings, "\\d+") as (string | null)[];
  ...
  // Test regex replace
  const replacedNumbers = str.replace(strings, "\\d+", "NUM") as string[];
  expect(replacedNumbers).toEqual(["abcNUMdef456", "noNumbers", "NUM"]);

  const replacedAllNumbers = str.replaceAll(strings, "\\d+", "NUM") as string[];
