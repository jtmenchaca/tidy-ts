...
const dataXLSXSheet = await readXLSX(pathToXLSX, PersonSchema, { sheet: "Summary" }); // select specific sheet
...
- `readCSV(), readXLSX(), readJSON(), readParquet(), and readArrow()` - Read CSV, XLSX, JSON, parquet, and arrow all with Zod schema validation for strong typing
